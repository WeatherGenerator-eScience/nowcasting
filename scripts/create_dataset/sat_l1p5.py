...
source_config = {
    "path": os.path.join(BASE_DATA_DIR, "satellite/L_1p5"),
    "interval": 5,  # minutes
    "fill_value": 0.0,
    "chunk_size": 2,
    "shard_size": 1024,
    "vars": {
        'IR_016': {'dtype': np.float16, 'dims': (384, 352)},
        'IR_039': {'dtype': np.float16, 'dims': (384, 352)},
        'IR_087': {'dtype': np.float16, 'dims': (384, 352)},
        'IR_097': {'dtype': np.float16, 'dims': (384, 352)},
        'IR_108': {'dtype': np.float16, 'dims': (384, 352)},
        'IR_120': {'dtype': np.float16, 'dims': (384, 352)},
        'IR_134': {'dtype': np.float16, 'dims': (384, 352)},
        'VIS006': {'dtype': np.float16, 'dims': (384, 352)},
        'VIS008': {'dtype': np.float16, 'dims': (384, 352)},
        'WV_062': {'dtype': np.float16, 'dims': (384, 352)},
        'WV_073': {'dtype': np.float16, 'dims': (384, 352)},
    }
}
...
def generate_members_tar(file_path_or_obj, is_fileobj=False):
    with tarfile.open(name=file_path_or_obj if not is_fileobj else None,
        ...
        for member in sorted(tar.getmembers(), key=lambda member_info: member_info.name):
            ...
            if member.name.endswith('_hrv.nc'):
                continue
...
def process_source_sequentially(root, source_config, time_map):
    ...
    for file_path in source_files:
        ...
        for fileobj, timestamp in generate_members_tar(file_path):
            ...
            shard_idx_previous = previous_idx  // shard_size
...
def main():
    ...
    group = root.create_group("sat_l1p5")
    ...
    group.create_array('lat', shape=(125, 250), dtype=np.float16)
    group.create_array('lon', shape=(125, 250), dtype=np.float16)
